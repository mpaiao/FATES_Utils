...
# - sigma0.boot:   Scaling factor for heteroscedastic model for each realisation           #
#                  (bootstrap only)                                                        #
...
optim.lsq.htscd <<- function( lsq.formula
   ...
   if (! skip.sigma){
      ...
      #----- Assign NULL values to skip heteroscedasticity. -------------------------------#
      sig.vars      = NULL
      names.sig.1st = NULL
      n.sig.1st     = 0
      #------------------------------------------------------------------------------------#
   ...
   names.1st    = c(names.lsq.1st,names.sig.1st)
   n.par        = length(x.1st)
   #---------------------------------------------------------------------------------------#
   ...
   ans$n.parameters  = n.par
   ans$df            = n.use - n.par
   ans$coefficients  = matrix( data     = NA_real_
                             , nrow     = n.par
                             , ncol     = 4L
                             , dimnames = list(names.1st,names.qual)
                             )#end matrix
   ans$sigma0        = rep( x = NA_real_, times = 4L)
   names(ans$sigma0) = names.qual
   #---------------------------------------------------------------------------------------#
   ...
   #---------------------------------------------------------------------------------------#
   #    Find the information criteria, using the corrections for small numbers (A78) for   #
   # AIC, and accounting for the full expression for Scharwz (Bayesian) information        #
   # criterion, following BA02.                                                            #
   #                                                                                       #
   # Akaike, H., 1978: On the likelihood of a time series model.                           #
   #    J. Roy. Stat. Soc. D-Sta., 27 (3-4), 217-235, doi:10.2307/2988185 (A78).           #
   #                                                                                       #
   # Burnham, K. P., and D. R. Anderson, 2002: Model Selection and Multimodel Inference: A #
   #    practical information -- theoretic approach. 2nd ed., Springer New York, New York, #
   #    NY, U.S.A., doi:10.1007/b97636 (BA02).                                             #
   #---------------------------------------------------------------------------------------#
   ans$AIC   = 2*(n.par - ans$support  + n.par*(n.par+1)/(n.use-n.par-1))
   ...
   if (err.short %in% "h"){
      ...
      if ("try-error" %in% is(se)){
         ...
         names(se)            = NULL
         ans$coefficients[,2L] = se
         ans$coefficients[,3L] = ans$coefficients[,1L] / ans$coefficients[,2L]
         ans$coefficients[,4L] = 2.0 * pt(-abs(ans$coefficients[,3L]),df=ans$df)
      }#end if
      #------------------------------------------------------------------------------------#
   }else{
      #------------------------------------------------------------------------------------#
      ...
      coeff.boot   = matrix( data     = NA_real_
                           , nrow     = n.boot
                           , ncol     = n.par
      ...
      tindex.boot  = matrix( data = NA_real_
                           , ncol = n.boot
                           , nrow = nrow(opt.data)
                           , dimnames = list(rownames(opt.data),NULL)
                           )#end matrix
      tobser.boot  = matrix( data = NA_real_
                           , ncol = n.boot
                           , nrow = nrow(opt.data)
                           , dimnames = list(rownames(opt.data),NULL)
                           )#end matrix
      tpred.boot   = matrix( data = NA_real_
                           , ncol = n.boot
                           , nrow = nrow(opt.data)
                           , dimnames = list(rownames(opt.data),NULL)
                           )#end matrix
      tsigma.boot  = matrix( data = NA_real_
                           , ncol = n.boot
                           , nrow = nrow(opt.data)
                           , dimnames = list(rownames(opt.data),NULL)
                           )#end matrix
      ...
      while (ib < n.boot){
         ...
         if (success){
            ...
            tindex.boot [,ib] = idx
            tobser.boot [,ib] = opt.data[[yname]][idx]
      ...
      #     Copy the results to ans.                                                       #
      #------------------------------------------------------------------------------------#
      ans$coefficients[,2L] = apply(X=coeff.boot,MARGIN=2L,FUN=sd  )
      ans$coefficients[,3L] = ans$coefficients[,1L] / ans$coefficients[,2L]
      ans$coefficients[,4L] = 2.0 * pt(-abs(ans$coefficients[,3L]),df=ans$df)
      ans$sigma0      [ 2L] = sd(sigma0.boot)
      ans$sigma0      [ 3L] = ans$sigma0[1L] / ans$sigma0[2L]
      ans$sigma0      [ 4L] = 2.0 * pt(-abs(ans$sigma0[3L]),df=ans$df)
   ...
   if (! skip.sxobs){
      ...
      #----- Initialise arrays that will store the data. ----------------------------------#
      coeff.sxobs   = matrix( data     = NA_real_
                            , nrow     = n.sxobs
                            , ncol     = n.par
                            , dimnames = list(NULL,names(x.1st))
                            )#end matrix
   ...
   if (! skip.syobs){
      ...
      #----- Initialise arrays that will store the data. ----------------------------------#
      coeff.syobs   = matrix( data     = NA_real_
                            , nrow     = n.syobs
                            , ncol     = n.par
                            , dimnames = list(NULL,names(x.1st))
                            )#end matrix
   ...
   if (err.short %in% "b"){
      ...
      #------------------------------------------------------------------------------------#
      #     Find summary statistics for training data of each iteration.                   #
      #------------------------------------------------------------------------------------#
      tresid.boot    = mapply( FUN = "-"
                             , e1  = as.data.frame(tobser.boot)
                             , e2  = as.data.frame(tpred.boot )
                             , SIMPLIFY = TRUE
                             )#end mapply
      ...
      #------------------------------------------------------------------------------------#
      xval.boot     = ifelse(test=is.finite(xval.boot),yes=xval.boot,no=NA_real_)
      ...
      mae.xval      =   apply(X=xval.resid,MARGIN=1,FUN=MeanAD              ,na.rm=TRUE)
...
predict.lsq.htscd <<- function( object
   ...
   n.par        = length(x.lsq) + as.integer(! is.null(sig.formula) ) * length(x.sig)
   #---------------------------------------------------------------------------------------#
...
print.lsq.htscd <<- function(object){
   ...
   #----- Report coefficients. ------------------------------------------------------------#
   p.value          = object$coefficients[,4L]
   coeff.table      = as.data.frame(object$coefficients)
   coeff.table[,1L] = sprintf("%g",signif(coeff.table[,1L],5L))
   coeff.table[,2L] = sprintf("%g",signif(coeff.table[,2L],5L))
   coeff.table[,3L] = sprintf("%g",signif(coeff.table[,3L],4L))
   coeff.table[,4L] = ifelse( test = p.value %lt% 1.e-16
                            , yes  = "< 1e-16"
                            , no   = sprintf("%g",signif(p.value,3L))
                            )#end ifelse
   #---------------------------------------------------------------------------------------#
   ...
   #---- Report sigma0. -------------------------------------------------------------------#
   if (is.na(object$sigma0[2L])){
      #---- Hessian, no error estimate provided. ------------------------------------------#
      sigma0.table = sprintf("%g",signif(object$sigma0[1L],4))
      #------------------------------------------------------------------------------------#
   }else{
      #---- Standardise output. -----------------------------------------------------------#
      s0.p.value        = object$sigma0[4L]
      sigma0.table      = as.data.frame(object$sigma0)
      sigma0.table[,1L] = sprintf("%g",signif(sigma0.table[,1L],5L))
      sigma0.table[,2L] = sprintf("%g",signif(sigma0.table[,2L],5L))
      sigma0.table[,3L] = sprintf("%g",signif(sigma0.table[,3L],4L))
      sigma0.table[,4L] = ifelse( test = s0.p.value %lt% 1.e-16
                                , yes  = "< 1e-16"
                                , no   = sprintf("%g",signif(s0.p.value,3L))
                                )#end ifelse
      #------------------------------------------------------------------------------------#
      ...
      sig.brks            = c(-Inf,0.001,0.01,0.05,0.1,Inf)
      sig.symbs           = c("***","**","*",".","")
      sigma0.table$signif = sig.symbs[as.numeric(cut(p.value,breaks=sig.brks))]
      #------------------------------------------------------------------------------------#
   }#end if (is.na(object$sigma0[2L]))
   #---------------------------------------------------------------------------------------#
   ...
   #---------------------------------------------------------------------------------------#
   bias      = sprintf("%g",signif(object$goodness$bias     ,4L))
   sigma     = sprintf("%g",signif(object$goodness$sigma    ,4L))
   rmse      = sprintf("%g",signif(object$goodness$rmse     ,4L))
   r.squared = sprintf("%g",signif(object$goodness$r.squared,3L))
   n         = object$goodness$n
   ...
   cat0("")
   if (is.na(object$sigma0[2L])){
      cat0("Sigma 0: ",sigma0.table)
   }else{
      cat0("Sigma 0:")
      print(sigma0.table,quote=FALSE)
      cat0(kplot)
   }#end if (is.na(object$sigma0[2L]))
   cat0("")
...
evaluate.lsq.htscd <<- function(x,lsq.formula,sig.formula,data,sigma0=NULL,...){
   ...
   #---------------------------------------------------------------------------------------#
   if (is.null(sig.formula)){
      #----- Homoscedastic model. Lamba must be 1. ----------------------------------------#
      sigma0 = sqrt( sum(yres^2,na.rm=TRUE) / (n.use - n.par) )
      sigma  = rep(x=sigma0,times=length(yres))
      lambda = sigma / sigma0
      #------------------------------------------------------------------------------------#
   }else if (is.null(sigma0)){
      #----- Heteroscedastic model, sigma0 is not known. ----------------------------------#
      sigbar = sqrt( sum(yres^2,na.rm=TRUE) / (n.use - n.par) )
      lambda = eval(expr=parse(text=sig.expr),envir=modeval)
      sigma0 = sqrt( sigbar^2 / mean(lambda^2) )
      sigma  = sigma0 * lambda
      #------------------------------------------------------------------------------------#
   }else{
      #----- Heteroscedastic model, sigma0 is known. --------------------------------------#
      lambda = eval(expr=parse(text=sig.expr),envir=modeval)
      sigma  = sigma0 * sqrt(lambda)
      #------------------------------------------------------------------------------------#
   }#end if
   #---------------------------------------------------------------------------------------#
   ...
   ans = data.frame( yhat   = yhat   # Fitted value
                   ...
                   , sigma0 = sigma0 # Reference standard deviation of residual
                   , lambda = lambda # Heteroscedasticity factor of SD of residual
                   )#end data.frame
   #---------------------------------------------------------------------------------------#
