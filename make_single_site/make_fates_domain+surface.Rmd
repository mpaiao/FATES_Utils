...
```{r, label='input-url'}
url_surfgrid = "https://svn-ccsm-inputdata.cgd.ucar.edu/trunk/inputdata/lnd/clm2/surfdata_map/release-clm5.0.18/surfdata_0.9x1.25_hist_16pfts_Irrig_CMIP6_simyr2000_c190214.nc"
```
...
# Download data in case they are not found.  We wrap the command in try because often the download
# gives an error message because the file size does not correspond to the expected, even though
# the file downloads fine.  If the file is actually corrupted, it will fail when we try to open it.
if (! file.exists(input_surfgrid)){
  dummy = try(download.file(url=url_surfgrid,destfile=input_surfgrid),silent=TRUE)
...
# Set output path to be the same as the suffix
output_path = file.path(here_path,site_tag)
dummy       = dir.create(output_path,recursive=TRUE,showWarnings=FALSE)

# Domain file.  In case the file exists, remove it.
output_domain   = file.path(output_path,paste0("domain_lnd_",site_tag,".nc"))
if (file.exists(output_domain)) file.remove(output_domain)

# Surface data file
output_surfdata = file.path(output_path,paste0("surfdata_",site_tag,".nc"))
...
wlon = (clon - dxy) %% 360.
elon = (clon + dxy) %% 360.
slat = max(-90.,clat - dxy)
nlat = min(+90.,clat + dxy)
```
