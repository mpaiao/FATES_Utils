...
* **output_path**. Directory to where FATES drivers will be written.

```{r, label='path-settings'}
here_path   = getwd()
input_file  = file.path(here_path,"gyf_met-driver_2004-2019.csv")
output_path = file.path(here_path,"fates_met_driver")
```
...
tstampz = make_datetime( year=yearz+1,month=1,day=1) - dtstamp
```
...
# Site description, which will also be a sub-directory
site_desc = paste0(xid,"_",dxy_lab,"_v",dat_version,"_c",ymd_lab)
site_path = file.path(output_path,site_desc)
metd_path = file.path(site_path,"CLM1PT_data")
...
outlon  = site_lon %% 360.
outlat  = site_lat
edge_w  = (site_lon - 0.5 * dxy) %% 360.
edge_e  = (site_lon + 0.5 * dxy) %% 360.
edge_s  = site_lat - 0.5 * dxy
edge_n  = site_lat + 0.5 * dxy
```
...
for (m in sequence(nmet)){
   ...
   }#end for (v in sequence(nvarinfo))

   # Create file
   nc_conn = nc_create(filename=nc_file,vars=nc_vlist,verbose=FALSE)

   # Put coordinates and tower height to the netcdf
   ...
   # Specify calendar for dimension variable time.
