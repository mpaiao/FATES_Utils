...
case_info = tidyr::tribble( ~idx, ~name                        , ~model     , ~desc                        , ~tstampa     , ~tstampz
                          ,    6, "D0006_ParacouEGHD_CLM_FATES", "CLM-FATES", "Paracou Evergreen-Deciduous", NA_character_, NA_character_
                          ,    7, "D0007_TanguroEGHD_CLM_FATES", "CLM-FATES", "Tanguro Evergreen-Deciduous", NA_character_, NA_character_
                          )#end tribble
...
pftinfo = tidyr::tribble( ~id, ~key   , ~short , ~desc                          , ~colour
                        ,   1, "pft01", "EvGn" , "Evergreen Tropical Tree"      , "#00BFC4"
                        ,   2, "pft02", "HyDd" , "Hydro-deciduous Tropical Tree", "#F8766D"
                        )#end tidyr::tribble
```
...
Turn data matrices into molten tibble objects.  These are compatible with `ggplot` and are the preferred data structure for analysing data efficiently using `tidyverse`.
```{r, label='melt-data'}
